...
Two middleware functions by default which are are located in the `track-server.middleware` namespace.
...
See the `:middleware` key of the `app` definition located in the `track-server.handler` namespace to manage the enabled middleware.
